...
| 4 avril 2026 | 1.3.2 | Audit quotidien approfondi. 7 branches obsolètes (à supprimer). Sécurité **5.5/10** : 2 critiques (injection shell), 8 hautes (routes publiques, uploads, clé PCM), 7 vuln npm. Tests 4 personas : **40 bugs dont 10 BLOQUANTS** — 3 modules cassés : Dashboard (colonnes SQL inexistantes), WorkHours (routes+champs+validation incompatibles), ProduitsFinis (création+affichage). Module Employees dégradé (position_name, CDDI). Note globale **5.3/10**. 179 commits |
