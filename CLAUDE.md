...
| 4 avril 2026 | 1.3.2 | Audit quotidien : 7 branches obsolètes re-détectées (toutes anciennes versions de main, à supprimer). Audit sécurité (6.8/10) : 3 critiques (injection shell admin-db, 2 injections SQL), 7 vuln npm (5 HIGH). Tests 4 personas approfondis : 25 bugs dont 7 BLOQUANTS (3 nouveaux : dashboard.js colonnes SQL inexistantes kg_entree/quantity/reference + 4 récurrents : Socket.IO GPS, ProduitsFinis, Production KPIs). Dashboard principal entièrement cassé. Note globale 6.4/10. 179 commits |
