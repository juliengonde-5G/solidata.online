...
| 4 avril 2026 | 1.3.2 | Audit quotidien le plus approfondi. 7 branches obsolètes. Sécurité **5.5/10** : 2 critiques, 8 hautes, 7 vuln npm. Tests 4 personas : **56 bugs dont 14 BLOQUANTS** — 4 modules cassés : Dashboard (SQL crash), WorkHours (routes+champs incompatibles), Mobile Chauffeur (CHECK constraint `returning`, TourSummary vide, km_end perdu, GPS mismatch+noms champs), ProduitsFinis (création+affichage). Workflow chauffeur **fondamentalement cassé**. Note globale **5.0/10**. 179 commits |
