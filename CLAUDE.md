...
| 4 avril 2026 | 1.3.2 | Audit quotidien approfondi. 7 branches obsolètes re-détectées (à supprimer). Audit sécurité **5.5/10** : 2 critiques (injection shell admin-db), 8 hautes (10 routes publiques sans auth, 3 uploads sans fileFilter, uploads publics, clé PCM hardcodée), 7 vuln npm. Tests 4 personas : **25 bugs dont 7 BLOQUANTS** (3 nouveaux dashboard.js SQL crash + 4 récurrents GPS/ProduitsFinis/Production). Dashboard principal cassé, ProduitsFinis inutilisable. Note globale **5.9/10**. 179 commits |
