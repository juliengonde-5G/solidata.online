...
| 6 avril 2026 | 1.3.2 | Audit quotidien : 11 commits orphelins réintégrés (design system lot 1, fix vue mv_cav_stats). Repo propre (1 branche). Sécurité **5.5/10** stable : 10 vuln npm (8 HIGH), mot de passe DB par défaut, injection shell admin-db. **36 bugs dont 6 BLOQUANTS actifs** — 5 modules cassés : Dashboard (SQL crash), WorkHours (endpoints incompatibles), ProduitsFinis (mismatch champs), GPS temps réel (Socket.IO), LiveVehicles. 4 bugs bloquants récurrents depuis 02/04. Note globale **5.2/10**. 155 commits |
