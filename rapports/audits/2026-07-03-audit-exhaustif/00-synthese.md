...
- ✅ **27 bloquants/majeurs confirmés** (§2, deux passes) — corrigés, testés et poussés. Couvre insertion IA, planning, RGPD candidats, radar freins, types produit/CO₂, webhook boutiques, PII véhicules, clé PCM, KPIs RH, scorecard/objectifs, ré-import VAK.
- ☐ **Un seul bloquant volontairement différé** : les **14 endpoints `tours/*-public` non authentifiés** (mobile chauffeur). Ils prennent un `:id` de tournée énumérable et acceptent des écritures (démarrer/terminer, pesées, incidents) sans token. Le mobile les appelle en `fetch()` sans authentification → **la sécurisation exige une bascule coordonnée mobile + backend** (token de tournée signé, comme le `qr_token` véhicule), pas un correctif isolé. À planifier comme un mini-chantier P1.
