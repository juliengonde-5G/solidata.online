...
- ☐ **Reliquat connexe** (rapport 03 A7, non traité) : `sync.js` pousse les scans QR vers `/tours/:id/scan` et le GPS vers `/tours/gps-batch` — deux endpoints **inexistants** → 404 → données purgées. À rebrancher (`/scan-public` existe ; l'endpoint GPS batch est à créer). Mini-tâche indépendante.
