...
- **Tableau des freins enrichi (d)** : **non commencé** au 14/09/2026 — `utils/insertion-freins-export.js` n'a pas été modifié depuis la PR A. Les colonnes BRSA/catégorie France Travail/référent unique/évolution des freins par axe demandées par l'autorité (§ 5.4 du contrat) restent à ajouter.
- **Reporting RH** (§ 5.5 du contrat) : `performance.js` remplace l'ancien `insertion.parcours_termines/total` par le nouveau dénominateur (voir plus haut) ; `ReportingRH.jsx`/`PerformanceDashboard.jsx` non vérifiés à cette date.
- **Documentation (lot 8)** : `docs/MATRICE_QUI_VOIT_QUOI_INSERTION.md` (nouveau, une page), `docs/PRESENTATION_AUTORITE_DEMONSTRATION.md` (nouveau, déroulé des cinq conditions), `docs/PIECES_HORS_LOGICIEL_INSERTION.md` (nouveau), `docs/PRESENTATION_AUTORITE_INSERTION.md`, `docs/NOTE_CERTIFICATEURS_INSERTION.md`, `docs/GUIDE_CIP_INSERTION.md` (cas 29-32), `docs/GUIDE_CIP_CONFORMITE_FSE.md` (§ 13) mis à jour — chacun distinguant explicitement le calcul serveur (livré) de l'écran (non livré).
