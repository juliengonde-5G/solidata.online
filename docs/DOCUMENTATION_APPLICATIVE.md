...
- **Notice d'information préalable** (2.45.0) : avant la **première question**, l'écran de passation présente une notice en **langage simple (FALC)** — à quoi sert le questionnaire et à quoi il ne sert pas (il **n'est jamais un critère de sélection**), qui voit le résultat, **les deux durées de conservation** (voir ci-dessous), les droits de la personne et comment les exercer. Le candidat **confirme l'avoir lue** ; la confirmation est **horodatée sur sa session** (`pcm_sessions.notice_acceptee_at`) et **`POST /submit` refuse (409 `PCM_NOTICE_NON_CONFIRMEE`)** sans elle — la garde est côté serveur, pas seulement à l'écran, et vaut aussi pour une soumission par un agent authentifié : la règle porte sur la passation, pas sur celui qui appuie sur le bouton. Un bouton « Je ne souhaite pas répondre » mène à un écran **sans reproche** qui indique vers qui se tourner ; le test ne peut alors pas commencer. Les sessions ouvertes avant cette version verront la notice à leur prochaine ouverture — le parcours se répare de lui-même. Le texte est une **source unique** (`frontend/src/utils/pcm.js`, `PCM_NOTICE_INFORMATION`), dont les mentions du PDF candidat sont **dérivées** (jamais recopiées : un délai modifié à un endroit ne peut pas laisser l'autre annoncer l'ancien).
- **Restitution au candidat** (2.45.0, art. 15 RGPD) : depuis l'écran de fin de test, la personne peut **imprimer ou enregistrer son résultat** (bouton « Imprimer mon résultat »), par le **même jeton de session** que sa passation — elle n'a pas de compte, ce jeton est sa seule pièce d'identité, et il ne donne accès qu'à **sa** propre session. Le document reprend la **mise en page A4 existante** (`openPrintWindow`, aucun nouveau générateur) et lui rend ce que le questionnaire dit **de sa manière de communiquer** (type principal, ce à quoi elle est attentive, la façon de lui parler qui marche, ses points forts, ce qui l'aide à bien travailler, les autres profils qui lui ressemblent), avec la mention de méthode et les mentions de conservation et de droits. Il **exclut** l'indicateur de cohérence des réponses, les paliers de stress rédigés en vocabulaire clinique, le « driver », les « masques » et le guide manager — **deux verrous** : le serveur ne les envoie pas (projection en liste blanche `restitutionCandidat`) et le générateur ne sait pas les afficher. Chaque restitution est **journalisée** (`rgpd_audit_log`, action `PCM_RESTITUTION_CANDIDAT`, sans utilisateur : c'est la personne elle-même).
- **Deux durées de conservation distinctes** (2.45.0) — contrepartie, demandée par le client, du **maintien de la passation dans le parcours de recrutement** (la recommandation de la déplacer après l'embauche a été écartée) :
  - les **réponses détaillées** (`pcm_answers`, les 20 choix) sont supprimées **30 jours** après la passation (`rgpd.pcm_reponses_retention_jours`), **pour tout le monde, personnes recrutées comprises** — le fondement n'est pas l'issue du recrutement mais la **minimisation** : une fois le profil calculé, les réponses item par item n'ont plus d'usage opérationnel ;
  - le **test entier** d'une personne **non recrutée** (session, réponses, rapport) est supprimé **90 jours** après la passation (`rgpd.pcm_non_recrute_retention_jours`, 2.44.0) ;
  - le test d'une personne **recrutée** suit son dossier salarié et disparaît à l'anonymisation de celui-ci.
  - **Conséquence assumée** : passé 30 jours, le script `reparer-rapports-pcm.js` ne peut plus reconstruire un rapport chiffré devenu illisible (il le recalculait **depuis les réponses**) ; l'export PDF technique, lui, **nomme l'absence** au lieu d'afficher un tableau vide. Les types de base et de phase sont stockés en clair et subsistent dans tous les cas.
- **Encart de méthode obligatoire** (2.43.0, **déplacé en 2.45.0 sur l'écran de notice** qui précède la passation — il y est lu au moment où la personne décide de répondre, et n'est plus affiché deux fois de suite), également présent sur la fiche profil et sur les deux PDF : rappelle que le questionnaire est un outil interne d'aide au dialogue inspiré du Process Communication Model, **non validé scientifiquement**, et qu'il ne doit **jamais fonder seul une décision de recrutement ou d'orientation**.
...
- **Chiffrement** : le rapport d'analyse est chiffré AES-256-GCM en base (`pcm_reports.encrypted_report`) ; les 20 réponses brutes (`pcm_answers`), elles, **ne sont volontairement pas chiffrées** — elles permettent de recalculer un rapport si la clé de chiffrement venait à changer, **pendant les 30 jours où elles sont conservées** (2.45.0).
...
- **Le candidat** garde sa propre restitution en fin de test (écran de passation) : c'est son résultat, la règle ci-dessus porte sur les agents de la structure. Depuis la 2.45.0 il peut en outre **l'emporter** (bouton « Imprimer mon résultat »).
...
| POST | `/api/pcm/sessions/:token/notice` | public (jeton de session) | **Confirmation de lecture de la notice d'information** (2.45.0) — horodatée, idempotente ; sans elle, `POST /submit` répond **409** |
| GET | `/api/pcm/sessions/:token/restitution` | public (jeton de session) | **Le candidat récupère SON résultat** (2.45.0, art. 15) — projection en liste blanche, sans l'indicateur de cohérence ni vocabulaire clinique ; journalisée |
...
**Contrats de test (2.45.0)** : `backend/tests/contract/pcm-notice-restitution-contract.test.js` verrouille la garde d'information préalable (409 sans confirmation, sur les deux chemins de soumission ; aucune réponse enregistrée ; trace horodatée et idempotente) et la restitution (le bon jeton rend le bon résultat, un autre jeton n'y accède pas, aucun indicateur de cohérence dans la sortie, journalisation sous son propre code, dégradation nommée quand le rapport chiffré n'est plus lisible). `backend/tests/contract/pcm-pdf-restitution.test.js` exécute les générateurs PDF **sur leur source réelle** (fiche technique sans réponses → absence nommée ; document candidat → mentions présentes, vocabulaire clinique absent). `backend/tests/unit/services/rgpd-purge-pcm-reponses.test.js` verrouille la purge des réponses (critère, périmètre élargi aux recrutés, journalisation conditionnelle, inscription au registre).
...
**Routes** : `/insertion` (Espace CIP — écran d'ouverture « Mes échéances »), `/insertion/actions`, `/insertion/audit` (Pilotage & indicateurs), `/insertion/renouvellement/:milestoneId` (écran encadrant, compte requis), `/eti/renouvellement/:token` (écran encadrant **public, sans compte**, PR C), `/admin/insertion` (réglages, ADMIN) | **Rôles** : ADMIN, RH, MANAGER (masquage des données sensibles pour MANAGER)
**API** : `backend/src/routes/insertion/` (routes.js, engine.js, freins-registry.js, masking.js, pmsmp-rules.js, **echeances.js**, **eti-public.js**, PR C) + `backend/src/services/echeances-cip.js`, `mon-parcours.js`, `rappels-rdv.js` (PR C) + `backend/src/routes/exports.js` (insertion-freins 23 col., insertion-synthese)
...
**Refonte CIP — PR A « Conformité immédiate » (2.52.0, septembre 2026)** — chantier `rapports/cip-refonte-2026-09-12/` (exigences de l'autorité de tutelle, personas, plan, maquettes, contrats, revues) :
- **Dossier administratif** (onglet de l'espace CIP, `routes/insertion/cadre.js`, `eligibilite.js`, `pieces.js`) : critères d'éligibilité IAE **référencés** depuis un référentiel administrable (14 critères seedés ; les justificatifs restent sur les Emplois de l'inclusion — jamais recopiés), Pass IAE (numéro, dates, **statut recalculé serveur** : actif / suspendu / prolongé / expiré / inconnu, événements de suspension et de prolongation), orienteur / prescripteur habilité / **référent unique** (la structure est **structure d'accueil**, pas référent unique : décision direction du 12/09/2026), actualisation France Travail, **statuts sociaux** (BRSA, catégorie France Travail A-G) **ADMIN/RH strict** — un MANAGER reçoit une projection sans ces clés —, dérogation CDDI, **pièces signées** dont la structure est seule dépositaire (entretien signé, convention PMSMP, accusé de remise ; BYTEA, vérification par octets d'en-tête, servies authentifiées `no-store` + `nosniff`, chaque consultation journalisée), bloc « à copier sur les Emplois de l'inclusion ». Lectures et écritures journalisées au registre RGPD.
- **Cofinancement FSE+** (`routes/insertion/projets.js`, `fse.js`, `conformite.js`, `routes/exports-fse.js`, `services/fse-participants.js`, `utils/fse-schema.js`) : **projets cofinancés** (ASI 2026-2027 et postes CIP en OCS 2026-2027 seedés, participants rattachés par un geste daté de la CIP, postes et quotités), questionnaire d'**entrée** typé (liste blanche d'items, validation serveur, **pré-remplissage par déduction** des rubriques du diagnostic avec la source affichée — la CIP confirme ou corrige), **sortie** enregistrée dans `insertion_fse_sorties` depuis le bilan de sortie OU **sans bilan** (personne partie sans entretien), relevé à **+6 mois** (dont « injoignable »), **dossier de conformité** en 9 pièces par salarié et vue transversale « Dossiers FSE+ — pièces à compléter » par projet, alertes J+15 / J+25 après la fin de contrat (job `checkFseSortiesNonRenseignees`), **export FSE+ participants** en 29 colonnes dictées par l'autorité (une colonne par item, jamais de JSON, `Délai de saisie (jours)` compté depuis la sortie de l'opération), **refusé en 409 à zéro ligne** et journalisé avant envoi ; bilan d'exécution agrégé.
- **Entretiens** : durée (chips 15/30/45/60/90 min, défaut par type paramétrable `insertion.duree_entretien_defaut`), présence / absent / excusé, motif d'absence **facultatif** (jamais imprimé « injustifiée »), référence de pièce.
- **Correctifs** : l'ancien export FSE+ sortait **toujours vide** (colonnes inexistantes sous un `catch` muet) — retiré ; export complet Insertion journalisé (`EXPORT_INSERTION_COMPLET`) avec en-tête de traçabilité et refus à zéro ligne ; Pass IAE et éligibilité enfin **saisissables** (`PUT /employees/:id`) ; suivi post-sortie à **+6 mois** (`insertion.post_sortie_mois`) ; `nb_brsa` des états ASP conservé ; échelle des freins corrigée dans les guides.
- **Guides** : `docs/GUIDE_CIP_CONFORMITE_FSE.md` (pas à pas avec visuels), `docs/PRESENTATION_AUTORITE_INSERTION.md` (présentation à l'autorité de tutelle).

**Refonte CIP — PR B « Cadre RSA et temps d'accompagnement » (2.53.0, septembre 2026)** — empilée sur la PR A, chantier `rapports/cip-refonte-2026-09-12/` (décision de direction du 12/09/2026 : **Solidarité Textiles est structure d'accueil, pas référent unique** — elle n'écrit pas le contrat d'engagements réciproques, elle **alimente** le référent externe qui le tient) :
- **Deux types d'entretien** (`referent_modalite`, `conciliation_motifs` JSONB liste fermée côté validateur, `conciliation_issue` — CHECK `milestone_type` reconstruit par DO-scan) : **« Point avec le référent »** (modalité `tripartite` / `bilaterale`, **hors compteur d'entretiens** des indicateurs existants) et **« Entretien de conciliation (protection des droits) »** (le formulaire commence par les motifs légitimes — `sante`/`garde_enfant`/`transport`/`demarche_administrative`/`formation_emploi`/`deuil_famille`/`autre` —, issue `maintien`/`reprise`/`orientation`/`sans_suite`).
- **Compteur d'activité hebdomadaire** (`services/activite-hebdo-engine.js`, module PUR, semaines ISO à pivot jeudi réutilisant `effectifs-engine.js`) : `total_heures` = temps de travail (`employee_week_hours`) **+** minutes d'accompagnement (entretiens et actions réalisés) **+** jours de PMSMP — décision 4 : le temps de travail CDDI compte. **Une semaine sans relevé n'est jamais une semaine à zéro heure** (`sans_releve: true`, `sous_seuil: null`) ; l'alerte (`insertion.semaines_sous_seuil_consecutives`, défaut 2) ne se déclenche **jamais pendant un arrêt déclaré** et une semaine sans relevé interrompt la série ; l'indicateur `nb_semaines_sous_seuil`, lui, compte aussi les arrêts (c'est ce que demande l'autorité) — et une cohorte **sans aucune semaine relevée** ne rend pas 0 mais `null` avec sa note (correctif PR D : « 0 semaine sous 15 h » et « rien de relevé » sont deux affirmations différentes). Raisons catégorisées `arret > temps_partiel > absence > inconnue`. Exposé par `GET /insertion/rsa/:employeeId/activite`, badge d'en-tête + tableau détaillé du Dossier administratif (`ActiviteHebdo.jsx`) — **jamais les mots « seuil », « obligation », « insuffisant »** sur une surface visible du salarié.
- **Fiche pour le référent** (`services/fiche-referent.js`, table `insertion_alimentations_referent`) : `composerFicheReferent` renvoie une **liste blanche stricte de 9 rubriques** (identité/destinataire, situation d'emploi, activité, assiduité, freins hors santé/judiciaire, actions hors freins sensibles, objectifs en cours, prochaines échéances, mentions RGPD) — les axes `sante` et `judiciaire` sont **structurellement absents** de l'objet (aucune clé, pas même nulle) et les colonnes ne sont même pas lues en SQL. 409 `REFERENT_NON_DETERMINE` posé **avant** toute lecture si `employees.referent_unique_type = 'non_determine'`. L'aperçu (`GET /fiche-referent`) n'écrit rien ; la génération (`POST`) enregistre un **snapshot** (`contenu` JSONB, destinataire recopié à l'instant T) et trace une **double remise** (référent, et un exemplaire à la personne). PDF `pdf-referent.js` (pattern `openPrintWindow`).
- **Relevé d'assiduité** (`composerReleveAssiduite`, variantes `tiers`/`dossier`) : rendez-vous proposés/honorés, absences **par motif catégorisé uniquement** (`employee_leaves.type_category`, jamais `leave_type` en clair) — une absence sans motif s'imprime « motif non renseigné », **jamais « injustifiée »**.
- **Actualisation France Travail** (`insertion_actualisations_ft`, `UNIQUE(employee_id, mois)`) : `honoree BOOLEAN` **nullable sans défaut** — trois états (faite / non faite / non constatée), jamais un défaut `false` qui transformerait un mois non vérifié en manquement. `PUT /actualisations-ft/:mois` recalcule `employees.actualisation_ft_derniere_date`/`_rappels_non_honores` depuis la table (jamais incrémentés).
- **Bloc « Rendez-vous réguliers et rappels »** (`GET /insertion/rsa/echeances-periodiques`, `EcheancesRsaBloc` dans `CohortePanel`) : actualisations FT du mois, points référent dus, salariés « en dessous de 15 h » agrégés, **référents non déterminés en rouge**.
- **Temps d'accompagnement** (lot 4, `services/temps-engine.js` module PUR + `services/temps-accompagnement.js`, routes `/api/insertion/temps`, page `/insertion/temps`) : `insertion_temps_saisies` (temps hors salarié nommé — atelier collectif, réunion de projet) et `insertion_feuilles_temps` (une par intervenant et par mois, `lignes`/`totaux`/`coherence` **NULL tant que brouillon**, **figés au moment de la validation**). Lignes composées depuis les entretiens et actions réalisés **avec durée non nulle** (sans durée → aucune ligne, jamais une durée par défaut inventée) et rattachées au projet ASI (participant actif) ou OCS (poste actif de l'intervenant), sinon `HORS_PROJET`. Cohérence avec les congés (`jour_absence`, `depassement_contractuel`) **signalée, jamais bloquante**. Transitions **forward-only** `brouillon → validee_intervenant → validee_rh` (409 `AUTO_VALIDATION` si la RH est aussi l'intervenant), réouverture ADMIN motivée. Export CSV (c) : 6 colonnes en français, **jamais le nom du bénéficiaire** (identifiant interne uniquement, absent dès la composition), pied de traçabilité, 409 `EXPORT_VIDE`. `heuresAccompagnement({ annee })` alimente `gatherAuditKpis` en mode `soft` (indicateur n° 14 de l'autorité).
- **Réglages** (`insertion.cer_heures_min/_max`, `insertion.semaines_sous_seuil_consecutives`, `insertion.point_etape_referent_mois`, `insertion.feuille_temps_cloture_jour` — voir `docs/VARIABLES_APPLICATION.md`).
- **RGPD** : entrée de registre « Transmission d'informations au référent unique (RSA) — fiche pour le référent » (base légale « mission d'intérêt public — à confirmer par le DPO ») ; anonymisation d'un salarié — `DELETE` intégral de `insertion_alimentations_referent` et `insertion_actualisations_ft`, `insertion_feuilles_temps.lignes` conservées (pièce de financement ≥ 5 ans) avec les `employee_id` du salarié **remplacés par `null`** dans le JSONB ; 10 codes journalisés (`INSERTION_ACTIVITE_CONSULTATION`, `INSERTION_ASSIDUITE_CONSULTATION`, `INSERTION_FICHE_REFERENT_APERCU/_GENERATION/_CONSULTATION/_REMISE`, `INSERTION_ACTUALISATION_FT_MAJ`, `INSERTION_FEUILLE_TEMPS_VALIDATION/_REOUVERTURE`, `EXPORT_FEUILLE_TEMPS`).
- **Réserves à arbitrer** : base légale de la transmission au référent (DPO), AIPD de la PR A à compléter de ce traitement, questionnaire officiel MDFSE+ toujours en attente.

**Refonte CIP — PR C « Section CIP et documents du salarié » (2.54.0, 13 septembre 2026)** — chantier `rapports/cip-refonte-2026-09-12/` (contrats `20-contrats-techniques-PR-C.md`, réalisations `21-realisation-lot5.md` et `21-realisation-lot7.md`), empilée sur la PR B, deux lots à fichiers disjoints :

- **Lot 5 — « Mes échéances »** (`backend/src/services/echeances-cip.js`, `routes/insertion/echeances.js`) : `GET /api/insertion/echeances` compose en **un seul appel** — chaque source en `soft()`, jamais de 500 — l'écran d'ouverture de l'espace CIP : **obligations** (9 types en liste fermée `TYPES_OBLIGATIONS`, chacun portant sa cible d'onglet — `pass_iae`, `cddi_plafond` cumul ≥ 23 mois sans `cddi_derogation_motif`, `diagnostic_socle`, `sortie_fse_a_saisir`, `fse_entree_manquant`, `categorie_g` orange seulement, `suivi_6_mois`, `referent_unique`, `sous_15h` ligne agrégée), **organisation du suivi** (bilans en retard — libellés issus de la liste **fermée** des types de jalon, jamais du `titre` saisi par la conseillère, qui n'est plus sélectionné — RDV non planifié, renouvellements < 42 j avec leur `lien_eti` **rendu à ADMIN/RH ou au MANAGER référent de ce salarié uniquement** — correctif de sécurité PR C, constats M-01 et B-01/D-02 —, actions critiques), **rendez-vous réguliers et rappels** (extrait de `rsa.js` sans changer sa forme de réponse), **file active** (KPI) ; `typesPourRole(baseRole)` exclut les 4 familles sociales pour MANAGER **avant tout calcul** (elles ne sont pas filtrées, elles ne sont pas exécutées). `POST /echeances/report` (ADMIN/RH strict, `authorize` posé avant tout validateur) : report de `report_echeance_heures` (défaut 48 h), motif **obligatoire à partir du 2ᵉ report** (409 `MOTIF_REQUIS`, liste fermée `MOTIFS_REPORT`), table `insertion_echeance_reports` (aucun UNIQUE — le nombre de lignes dit qu'un dossier tourne en rond), journal bloquant `INSERTION_ECHEANCE_REPORT`. `GET /echeances/compteur` → `{ rouges }`, cache mémoire 60 s par (rôle, utilisateur, périmètre), non journalisé (la barre latérale l'appelle à chaque montage), panne → `{ rouges: 0, indisponible: true }` en 200. `niveauRisque()`/`niveauxRisqueCohorte()` : fonction **unique** consommée aussi par `GET /insertion` (la pastille de la liste ne peut pas contredire l'écran).
- **Lot 5 — file active** (`routes/insertion/routes.js`, `GET /`) : périmètre `sqlPerimetreFileActive` = `insertion_status = 'en_parcours'` OU (`termine` ET sortie < `file_active_terminees_mois` mois, défaut 7) ; **les permanents (`none`) ne sont jamais renvoyés** ; 18 champs ajoutés (`prochain_rdv`, `risque`, `projets[]`, `pass_iae_statut`, `referent_unique_type`, `brsa`…) ; `brsa` **non lu** pour un MANAGER (colonne absente de la requête SQL, pas masquée après lecture) ; `?inclure=tous`, `?mine=1`. **Correctif de sécurité M-07** (`24-correctifs-PR-C.md`) : la rémanence des parcours `termine` (fenêtre `file_active_terminees_mois`) et `?inclure=tous` sont désormais **réservés à ADMIN/RH** — un MANAGER ne reçoit que les parcours `en_parcours`, jamais les parcours terminés, et `?inclure=tous` n'a aucun effet pour lui (même règle appliquée au moteur d'échéances de `echeances-cip.js`).
- **Lot 5 — fiche en 4 onglets** (`InsertionParcours.jsx` 1 853 → 655 lignes, composants `components/insertion/EcheancesPanel.jsx`, `FileActive.jsx`, `OngletSituation.jsx`, `OngletSuivi.jsx`, `FreinsDeltas.jsx`) : **Situation** (freins — `RadarFreins` + `FreinsDeltas`, badges levé/stable/aggravé/non comparable par axe — puis note de profil, frise repliée, check-list, PMSMP, satisfaction, `DocumentsSalariePanel` du lot 7, bouton « Proposition de synthèse (IA) »), **Suivi** (entretiens & bilans, objectifs, actions, notes de suivi ADMIN/RH, `CompetencesETI` en encart repliable), **Dossier administratif** (inchangé + `RappelsConsentement` du lot 7 en bas via la prop `extra`), **Diagnostic** (§ suivant). L'onglet « Assistant IA » disparaît ; la sonde de connexion (« Tester la connexion IA ») part dans `AdminInsertion.jsx`. Fiche collaborateur (`Employees.jsx`, onglet « Parcours insertion ») réduite à un résumé + « Ouvrir dans l'espace CIP » (REC-UX-12, un seul chemin de saisie). Tous les `window.confirm`/`alert()` de `InsertionParcours.jsx`, `Employees.jsx`, `RenouvellementETI.jsx`, `DiagnosticForm.jsx` remplacés par `Modal`/`ConfirmDialog` (`grep` = 0).
- **Lot 5 — diagnostic socle + approfondissements** (`components/insertion/DiagnosticForm.jsx`) : `SOCLE` (7 rubriques dans l'ordre — cadre administratif & éligibilité, logement, santé, mobilité, situation & projet professionnels, expression du salarié + freins, **questionnaire FSE+ d'entrée en dernier**) et `APPROFONDISSEMENTS` (parcours & famille, budget, portefeuille & AFOM, style d'apprentissage, projet de formation — repliés, jamais comptés dans la complétude). Liste des champs du socle en **fichier partagé** `backend/src/data/diagnostic-socle-champs.json` (le contexte de build Docker du frontend ne contient pas `backend/` : la copie frontend `components/insertion/diagnostic-socle-champs.json` est **testée identique** par Jest plutôt qu'importée). MANAGER : rubrique FSE+ retirée.
- **Lot 5 — ETI par jeton public** (`routes/insertion/eti-public.js`, monté `/api/eti` **hors** `authenticate`, rate-limité 60 req/15 min) : colonnes `insertion_milestones.eti_token`/`eti_token_expires_at`/`eti_token_generated_by` (index unique partiel), `POST /insertion/renouvellements/:id/lien-eti` (ADMIN/RH ou MANAGER propriétaire, `crypto.randomBytes(16)`, **remplace** un jeton existant — c'est le mode de révocation, l'ancien lien meurt), `insertion.eti_token_validite_jours` (défaut 60). `GET /api/eti/renouvellement/:token` : 404 uniforme (jeton malformé, inconnu, ou mauvais type d'entretien), 410 `LIEN_EXPIRE`/`ENTRETIEN_CLOTURE`, réponse à **9 clés exactement**, jamais `employee_id`. `PUT` : liste blanche de 3 champs vérifiée avant toute lecture, `{ ok: true }` en réponse (jamais la ligne), journal `INSERTION_ETI_FORMULAIRE_JETON` (`user_id` NULL, IP + préfixe du jeton). `RenouvellementETI.jsx` devient la voie « pour les comptes » ; `EtiRenouvellement.jsx` (public, `axios` nu, hors `ProtectedRoute`) et `FormulaireETI.jsx` (trame partagée) sont nouveaux.
- **Lot 7 — documents pour la personne** (`services/mon-parcours.js`, `routes/insertion/salarie.js` — `authorize('ADMIN','RH')` en première ligne, avant tout validateur) : table `insertion_documents_salarie` (`contenu JSONB` = **snapshot**, jamais une vue recalculée ; `type` `mon_parcours`/`mon_recap`). `composerMonParcours` (9 clés : personne, structure, mes_engagements, engagements_structure, **mes_heures_semaine sans aucun seuil**, prochain_rdv sans le type d'entretien — sa date et son heure sont celles de `insertion_milestones.interview_date`, un `TIMESTAMP WITHOUT TIME ZONE` qui porte **déjà** l'heure MURALE de Paris (saisie au clavier telle quelle) : lues via `heureMurale`/`isoDate` et **jamais reconverties** — les convertir « vers Paris » ajouterait l'offset une seconde fois (correctif D-01/M-08) —, mon_referent, mes_documents_remis, genere_le) et `composerMonRecap` (7 clés : personne, structure, contrats, étapes **par type/catégorie fermés — jamais le titre libre saisi**, objectifs en compteurs, sortie, genere_le) — **liste blanche exportée** `MON_PARCOURS_CLES`/`MON_RECAP_CLES`, `CLES_INTERDITES` testée sur l'arbre complet (`frein_*`, `sante`, `judiciaire`, `brsa`, `ft_categorie`, `note`, `observations`, `commentaire*`, `motif*`, `presence`, `absence*`, `bilan_*`, `avis_*`, `description`). `GET/POST /:employeeId/mon-parcours|mon-recap` (aperçu = journal tolérant, sans écriture ; génération = snapshot + journal **bloquant** dans la transaction), `GET /documents`, `GET /documents/:id`, `PUT /documents/:id/remise` (409 si déjà tracée, `FOR UPDATE`).
- **Lot 7 — rappels de rendez-vous** (`services/rappels-rdv.js`, job `envoyerRappelsRdvSalaries`) : colonnes `employees.rappel_rdv_consent` (BOOLEAN **sans DEFAULT** — `NULL` = jamais demandé), `rappel_rdv_canal`, `rappel_rdv_destinataire`, `rappel_rdv_consent_at/_by` ; table `insertion_rappels_rdv` (`UNIQUE(milestone_id)`, `destinataire_masque` uniquement — jamais le contact en clair). `selectionnerRendezVous` : consentement, canal, destinataire et `is_active` dans le `WHERE` ; « demain » calculé **par PostgreSQL** en `Europe/Paris` (`(NOW() AT TIME ZONE 'Europe/Paris')::date + 1`) et comparé au **jour tel que STOCKÉ** dans `interview_date` (`::date`, **sans aucune conversion** — `interview_date` porte déjà l'heure murale de Paris ; la reconvertir ferait basculer un rendez-vous tardif au surlendemain et ne l'enverrait jamais, correctif D-03) ; **le type d'entretien n'est pas sélectionné**. `doitEnvoyerRappels(now, heure)` fonction pure sur l'heure murale de Paris (`Intl`, DST géré) — réglage `insertion.rappel_rdv_heure_envoi` (défaut 18) ; branché dans le tick horaire de `services/scheduler.js`. Envoi via `sendNotification` (gabarits `message_templates` catégorie `insertion_rappel_rdv`, seedés actifs, **prénom retiré du texte** depuis le correctif M-05 — variables `{date}`, `{heure}`, `{cip}` uniquement) ; un **message de vérification distinct** (catégorie `insertion_rappel_verification`, gabarits seedés à part) part au moment du recueil du consentement, journalisé sous le code propre `INSERTION_RAPPEL_VERIFICATION` ; sans clé Brevo, statut `dry_run` — jamais prétendu envoyé. `GET/PUT /:employeeId/rappels-consentement` (colonnes `employees` **et** `rgpd_consents` dans la même transaction ; un permanent ne peut pas consentir — 409 `HORS_PARCOURS` — mais un retrait reste toujours possible), `GET /:employeeId/rappels` (historique masqué). Purge `purgeRappelsRdv` (**10ᵉ purge** de `services/rgpd-purges.js`, seuil `insertion.rappels_retention_jours` **défaut 90** — ramené de 365 par le correctif de revue de sécurité, minimisation).
- **Réglages** (`backend/src/utils/insertion-settings.js`) : `insertion.eti_token_validite_jours` (60), `insertion.report_echeance_heures` (48), `insertion.file_active_terminees_mois` (7), `insertion.categorie_g_alerte_jours` (30), `insertion.rappel_rdv_heure_envoi` (18), `insertion.rappels_retention_jours` (**90**), `insertion.recap_neutralise` (**true** — neutralise, sur « Mon Récap » seulement, le libellé des entretiens de conciliation/point référent et la raison sociale d'une PMSMP ; ajouté par le correctif M-10).
- **RGPD** (`frontend/src/utils/rgpd-libelles.js`) : 10 codes — `INSERTION_ECHEANCES_CONSULTATION`, `INSERTION_ECHEANCE_REPORT`, `INSERTION_ETI_LIEN_GENERATION`, `INSERTION_ETI_FORMULAIRE_JETON`, `INSERTION_DOC_SALARIE_APERCU/_GENERATION/_CONSULTATION/_REMISE`, `INSERTION_RAPPEL_CONSENTEMENT`, `INSERTION_RAPPEL_ENVOI`. Aucune nouvelle entrée au registre art. 30 (l'ETI par jeton est un mode d'accès au traitement « accompagnement socio-professionnel » existant ; les rappels de rendez-vous, base légale consentement, en reçoivent une propre — sous-traitant Brevo, rétention = celle de la trace).
- **Anonymisation** (`services/anonymization.js`) : `insertion_documents_salarie` et `insertion_rappels_rdv` (DELETE — la trace de remise survit dans `rgpd_audit_log`, pas le contenu), `insertion_echeance_reports` (DELETE), `insertion_milestones.eti_token` → NULL, `employees.rappel_rdv_consent/_destinataire` → NULL.
- **Preuves (au moment de la livraison des deux lots)** : **Jest 243 suites / 4 898 tests verts, 0 échec** (+212 sur la PR B, dont 110 pour le lot 5 et 102 pour le lot 7), build Vite vert, `InsertionParcours.jsx` 1 853 → 655 lignes, `window.confirm`/`alert()` = 0 sur les fichiers concernés, 10 contre-épreuves par mutation (5 par lot) toutes restaurées.
- **Écarts au contrat, assumés et documentés dans les rapports de réalisation** : le fichier des champs du socle est **copié** (testé identique) plutôt qu'importé des deux côtés (contrainte du build Docker frontend) ; `AlertesBloc` reste en en-tête de fiche plutôt que dans l'onglet Situation ; le helper `cumulCddiMois` n'a pas été extrait (il est importé depuis `engine.js`, éviter une seconde règle pour un plafond légal) ; les entrées « Mon parcours » / « Mon Récap » d'un menu « Fiche PDF ▾ » en en-tête n'ont pas été posées (les deux documents restent accessibles depuis `DocumentsSalariePanel` de l'onglet Situation).
- **Revue de sécurité et debug PostgreSQL réel** (`22-revue-securite-PR-C.md`, `23-debug-postgres-PR-C.md`, correctifs appliqués dans `24-correctifs-PR-C.md`) : 2 bloquants, 10 majeurs, 12 mineurs (revue) + 1 bloquant, 2 majeurs, 2 moyens, 2 mineurs (debug), **31 constats sur 31 corrigés**. Le plus important : le **jeton ETI** ne sortait plus d'un `SELECT im.*` protégé par rôle mais d'un `SELECT im.*` non filtré (`masking.js`, désormais `stripSecrets` appliqué **avant** tout test de rôle) et le lien lui-même (`lien_eti`) était rendu à n'importe quel MANAGER (désormais réservé au référent/encadrant de CE salarié via `insertion-appartenance.js`, sinon ADMIN/RH) ; l'écran public de l'ETI ne renvoyait pas de liste blanche stricte (corrigé, `rempli_par` posé par le serveur) ; le SMS de rappel annonçait une heure décalée de deux heures (heure murale reconvertie à tort, D-01) et la sélection « demain » convertissait aussi le jour stocké (D-03) ; le titre libre d'un entretien fuyait par 3 requêtes hors liste fermée (M-01) ; l'anonymisation pouvait échouer en silence sur une colonne absente (M-03) ; un refus d'envoi Brevo pouvait être tracé « envoyé » (M-04). **Jest 244 suites / 4 928 tests verts**, **e2e PR C 136/136** sous `TZ=UTC` et `TZ=Europe/Paris`, non-régression PR A+PR B 180/180, 14 contre-épreuves par mutation. Détail complet, chiffres et arbitrages restant ouverts (direction/DPO) dans `24-correctifs-PR-C.md` § 7.

**Refonte CIP — PR D « Reporting autorité et présentation » (2.55.0, 14 septembre 2026)** — chantier `rapports/cip-refonte-2026-09-12/`, contrat `25-contrats-techniques-PR-D.md`, empilée sur la PR C, deux lots : **lot 6 « Reporting autorité »** (backend + frontend) et **lot 8 « Documentation et présentation »** (ce document et les cinq autres cités plus bas), suivis d'une revue de sécurité (`27`), d'un debug sur PostgreSQL réel (`28`) et de correctifs (`29` : 32 constats corrigés sur 32). État constaté dans le code après correctifs — **couches serveur et écran livrées** :

- **Dénominateur des sorties** (`services/sorties-engine.js`, module PUR, entrées injectées, testable sans base) : `calculerSorties()` calcule désormais un **dénominateur « méthode B »** = toutes les fins de parcours de la période (au lieu des seuls bilans de sortie classés), une ligne **« sortie non documentée »** pour les parcours terminés sans bilan classé, et — pour l'année 2026 seulement — la **méthode A historique en parallèle** (`methode_a`, `null` pour toute autre année) afin que la rupture de série soit annoncée plutôt que découverte. **Déjà consommé** par `gatherAuditKpis().sorties` (nouvelles clés `methode_b`, `non_documentees`, `rapprochement_asp`, `regles` ; les clés historiques inchangées) et par `routes/performance.js` (bloc `insertion.fins_parcours_annee`/`sorties`). **Affiché** : le bloc Sorties d'`AuditInsertion.jsx` rend la méthode B en premier (fins de parcours, classées, ligne « non documentées » en ambre), la méthode A à côté quand `methode_a_imprimee` est renseignée ; `ReportingRH.jsx` et `PerformanceDashboard.jsx` lisent `fins_parcours_annee` et `sorties.non_documentees`. `/exports/insertion-synthese` et `/insertion/reporting/dialogue-gestion` consomment la même fonction — jamais recopiée.
- **Synthèse de dialogue de gestion** (export (e) de la matrice de l'autorité) : `routes/insertion/reporting.js` expose `GET /api/insertion/reporting/dialogue-gestion` (aperçu, journal tolérant), `POST` (génération enregistrée en snapshot dans `insertion_dialogues_gestion`, journal **bloquant** dans la même transaction), `GET .../historique` et `GET .../:id` (rejoue un snapshot). Composition des neuf blocs par `services/dialogue-gestion.js`. Ouvert en lecture à **ADMIN/RH/MANAGER** — le seul document du module lisible en intégralité par un encadrant technique, parce qu'**aucune projection nominative n'existe dans son schéma** (liste blanche d'agrégats, k-anonymat à 5 personnes). Le **k-anonymat est structurel** (correctif B-01) : une seule passe sur le document entier (`appliquerKAnonymat`) retire tout entier compris entre 1 et k−1 sauf six effectifs bruts argumentés, ferme les canaux de reconstitution (taux miroirs, **suppression complémentaire**, comptes recopiés dans les phrases de méthode), rend `sous_seuil` comme un **compte par bloc** + `sous_seuil_total` (jamais le chemin de la case retirée) ; plancher 5, le réglage `insertion.k_anonymat_min` ne peut que durcir. Les **statuts sociaux agrégés** (BRSA, catégorie FT, référent, RQTH) ne sont jamais composés hors ADMIN/RH — ni ici ni par `/insertion/audit` (correctif B-02). L'**aperçu** journalise de façon **bloquante** (`INSERTION_DIALOGUE_GESTION_APERCU`, m-05) sans créer de snapshot ; le CSV (`EXPORT_DIALOGUE_GESTION`) de même ; zéro fin de parcours → 409 `EXPORT_VIDE`. Rétention du snapshot `rgpd.dialogues_gestion_retention_jours` 2 190 j (11ᵉ purge, `purgeDialoguesGestion`). **Affiché** : onglet « Dialogue de gestion » d'`AuditInsertion.jsx` → `DialogueGestionPanel.jsx` (Aperçu / Imprimer l'aperçu / Générer et enregistrer (PDF) — ADMIN/RH / CSV / historique rejouable en PDF), PDF A4 composé par `pdf-dialogue-gestion.js` (en-tête de traçabilité avec `APP_VERSION`, neuf blocs, page « Méthode »). **Le rendu du PDF n'a pas été éprouvé visuellement** (contrôle à faire).
- **Débouché d'une PMSMP** (S1/S7) : colonnes `insertion_pmsmp.debouche` (liste fermée : embauche chez l'accueillant/ailleurs, formation, poursuite du parcours, aucun, inconnu), `debouche_date`, `embauche_accueillant` (déduit du débouché, `null` si inconnu). `POST/PUT /pmsmp` les accepte et les valide. **Affiché** : `PmsmpPanel.jsx` propose le sélecteur « Débouché » + date sur toute immersion dont la date de fin est passée (`embauche_accueillant` déduit côté serveur, jamais saisi ; `pool.connect()` des handlers PMSMP ramené dans le `try`, m-01).
- **Orientation DORA et aide mobilisée sur une action CIP** : colonnes `cip_action_plans.dora_service/dora_url/dora_resultat` (résultat en liste fermée : orienté, pris en charge, refusé, sans suite) et `aide_nature/aide_organisme/aide_montant` (nature en liste fermée, montant `null` jamais 0). `POST/PUT /action-plans` les valide (`dora_url` https strict). **Affiché** : `ActionsPanel.jsx` rend un bloc repliable « Orientation DORA / aide mobilisée » sous chaque action. Les six champs sont inscrits dans `ACTIONS_TEXTE_LIBRE` de `masking.js` et **masqués sur l'axe santé pour l'encadrant** (correctif B-03) ; une **garde statique** confronte ce dictionnaire au schéma pour qu'une colonne ajoutée demain ne puisse pas l'oublier.
- **Catégories** : `PARTENAIRE_CATEGORIES` gagne `cms` ; `cip_action_plans.category` gagne `job_dating`, `formation` (déjà posés en PR A/correctif PR C D-07) puis **`formation_fle`** (PR D, « Cours de français » — distingue le frein linguistique d'une formation qualifiante) ; `services/mon-parcours.js` (`CATEGORIE_ACTION_LABELS`) et `frontend/src/components/insertion/pdf-referent.js` tenus à jour.
- **Tableau des freins enrichi (d)** : `utils/insertion-freins-export.js` passe de 23 à **45 colonnes** (48 avec `sensibles=1`) — les 23 du CDC intactes en tête, puis BRSA, catégorie FT, critères d'éligibilité (jamais l'art. 10), Pass IAE, référent unique, projet, prescripteur, semaines sous 15 h, et par axe la **valeur d'entrée + évolution** (`levé`/`stable`/`aggravé`/`non évalué` — jamais « stable » inventé quand la dernière évaluation manque, D-05) ; « Heures par semaine (quotité contractuelle) » ; dates par `date-iso.js` (les cinq dates reculaient d'un jour hors UTC, D-06) ; en-tête commenté en CSV (m-08), journal `EXPORT_INSERTION_FREINS_ENRICHI`. Code `EXPORT_VIDE` à zéro ligne.
- **Reporting RH** (§ 5.5 du contrat) : `performance.js` remplace l'ancien `insertion.parcours_termines/total` par le nouveau dénominateur (voir plus haut) ; `ReportingRH.jsx` et `PerformanceDashboard.jsx` affichent `fins_parcours_annee` et la ligne « non documentées ». Un mois sans aucun contrat CDDI vaut désormais `null` ETP et non 1,00 (D-04 : `COALESCE(…, 35)` sur la ligne vide d'un LEFT JOIN).
- **Documentation (lot 8)** : `docs/MATRICE_QUI_VOIT_QUOI_INSERTION.md` (nouveau, une page), `docs/PRESENTATION_AUTORITE_DEMONSTRATION.md` (nouveau, déroulé des cinq conditions), `docs/PIECES_HORS_LOGICIEL_INSERTION.md` (nouveau), `docs/PRESENTATION_AUTORITE_INSERTION.md`, `docs/NOTE_CERTIFICATEURS_INSERTION.md`, `docs/GUIDE_CIP_INSERTION.md` (cas 29-32), `docs/GUIDE_CIP_CONFORMITE_FSE.md` (§ 13) mis à jour — écrits d'abord pendant que le lot 6 se construisait (ils disaient l'écran « non livré »), puis **repassés après la livraison et les correctifs**, chaque chemin de clic vérifié dans le code. `APP_VERSION` déclarée dans le compose et les deux `.env.example` (les documents transmis portaient « 1.0.0 »).
- **Réglages** (`insertion-settings.js`) : `insertion.sorties_methode_double_annee` (2026), `insertion.k_anonymat_min` (5), `insertion.heures_annuelles_etp` (1820, repli si `effectifs.convention_<année>.heures_annuelles_etp` absent).
- **RGPD** : aucune donnée nominative nouvelle — le snapshot de la synthèse est agrégé, aucune entrée de registre supplémentaire n'est attendue. Codes de journal ajoutés : `INSERTION_DIALOGUE_GESTION_APERCU/_GENERATION/_CONSULTATION`, `EXPORT_DIALOGUE_GESTION`, `EXPORT_INSERTION_FREINS_ENRICHI`.

**Refonte CIP — PR E « Suivi Convergence (programme CVG) » (2.60.0, 25 septembre 2026)** — chantier `rapports/cip-refonte-2026-09-12/`, contrat `30-convergence-cvg-cartographie.md`, empilée sur la PR D. Un document **distinct** de la synthèse de dialogue de gestion ci-dessus : il s'adresse au réseau **Convergence France**, dans sa propre nomenclature (cinq types d'habitat, douze orienteurs, huit catégories de sortie), attendue en principe deux fois par an.

- **Schéma** (`migrations/insertion-convergence.js`) : `insertion_diagnostics` gagne `habitat_type` (5 valeurs CHECK, entre dans le **socle** J+30 — `diagnostic-socle-champs.json`), `parcours_rue`, `pension_invalidite`, `medecin_traitant` (ces trois derniers hors socle, approfondissements) ; `employees.orienteur_type` passe de 6 à 12 + 6 valeurs acceptées (colonne élargie VARCHAR(40), CHECK reconstruit idempotent, anciennes valeurs conservées lisibles) ; nouvelle table `insertion_sortie_cvg` (une ligne par parcours, `UNIQUE(employee_id, parcours_num)` : catégorie de sortie Convergence, parcours de soin, habitat à la sortie, RQTH/AAH/pension/médecin traitant/couverture santé à la sortie, accompagnement post-sortie) ; nouvelle table `insertion_cvg_ressources` (Partie 2 « moyens humains » — internes avec ventilation accompagnement/encadrement, ou mutualisées, en ETP, aucune source automatique) ; `insertion_dialogues_gestion` gagne `type` (`'dialogue'`/`'cvg'`, défaut `'dialogue'`) et `periode_debut`/`periode_fin` — **un instantané Convergence est un snapshot de cette même table**, pas une nouvelle table de pièces transmises. Entrée art. 30 « Reporting Convergence (programme CVG) » posée une fois, base légale « à confirmer par le DPO ».
- **Transcodages** (`utils/convergence-cvg-referentiels.js`, module PUR, aucune E/S) : `transcoderHabitat` (statut de logement → habitat CVG, `heberge` **ambigu → null**, jamais deviné), `transcoderOrienteur` (les 12 valeurs passent telles quelles, `departement_cms`→services sociaux du Département, `ccas`→autre acteur local d'accompagnement, `autre`→null), `transcoderSortieType` (type de sortie du bilan → catégorie CVG, `sans_suite`→sans nouvelles **en approximation annoncée**), `transcoderFrein` (8 des 9 freins SOLIDATA ; **`numerique` sans équivalent, jamais transmis**), `sexeCvg`/`trancheAgeCvg`/`niveauFormationCvg`. Même fichier consommé par la migration (listes des CHECK), le composeur, le comparateur et la route de situation de sortie — jamais recopié.
- **Backend** : `services/convergence-cvg.js` (`composerCvg` — Partie 1 le public, Partie 2 les moyens humains, sorties en deux tableaux jumeaux emploi/hors emploi avec évolution des freins/logement/santé entrée→sortie ; `comparerCvg` PUR — écarts en nombre et en points de pourcentage + lecture rédigée constatant une évolution jamais sa cause ; `cvgVersCsv`, `composerCompletude` — liste **nominative** interne, jamais dans le document transmis). `routes/insertion/convergence.js` (monté `/insertion/convergence`, **ADMIN/RH strict**, refus redoublé au routeur) : `GET /apercu` (journal bloquant), `POST /generer` (snapshot + journal dans la même transaction, 409 `EXPORT_VIDE` si aucun accueilli ni sorti, 503 `SOURCE_ILLISIBLE` si cohorte et sorties sont illisibles — m-07), `GET /parametres` (seuil de confidentialité et transmission de la justice en vigueur), `GET /historique`, `GET /snapshot/:id`, `GET /comparaison` (deux instantanés ou deux périodes à la volée), `GET /completude`, `GET /csv`, `GET|PUT /situation-sortie/:employeeId` (une **proposition déduite** du dossier avec provenance en toutes lettres, jamais écrite sans confirmation), CRUD `/ressources`. Famille `sortie_cvg` de `services/echeances-cip.js` : obligation rouge « Situation de sortie Convergence à saisir » 30 j après la fin de parcours (`insertion.cvg_sortie_delai_jours`), sociale (ADMIN/RH strict), reportable 48 h comme les autres obligations du bloc, jamais acquittable.
- **Frontend** : `ConvergenceCvgPanel.jsx` (onglet « Convergence (CVG) » d'`AuditInsertion.jsx` — Document/Aperçu/Générer/CSV/Historique, Comparer deux périodes, À compléter, Moyens humains), `pdf-convergence-cvg.js` (impression A4 par fenêtre, même mécanisme que le PDF de dialogue de gestion), `convergence-cvg-structure.js` (mise en forme partagée panneau/PDF). `SituationSortieCvg.jsx` : bloc « Situation à la sortie (Convergence) » du bilan de sortie (`EntretienForm.jsx`, section « Sortie & documents »), **enregistrement à part** du verrouillage de l'entretien — modifiable dans les 30 j suivant la clôture. `DiagnosticForm.jsx` : rubrique Logement (habitat CVG + proposition à confirmer depuis le statut de logement, parcours de rue), rubrique Santé (pension d'invalidité, médecin traitant), niveaux de formation 6/7/8 distingués (`niv6plus` conservé en lecture). `DossierAdministratif.jsx` : liste d'orienteurs à 12 valeurs + anciennes valeurs lisibles. Libellés partagés dans `frontend/src/components/insertion/freins.js` (`CVG_HABITAT_LABELS`, `CVG_SORTIE_LABELS`, `ORIENTEUR_LABELS`).
- **Doctrine** : remplissage **permanent**, jamais une campagne — chaque information se saisit là où elle naît (diagnostic à l'entrée, bilan de sortie à la sortie), l'onglet Convergence ne fait que composer, comparer et transmettre. Un instantané enregistré est un **snapshot rejouable à l'identique**, distinct de ce que le dossier dit aujourd'hui. **Seuil de confidentialité propre** (correctif B-01, 2.60.0) : `insertion.cvg_k_min`, défaut 5, plancher de code 1 (le DPO peut poser 1 pour reproduire le format brut du réseau, effectifs de 1 et 2 compris) — un tableau des sortis de moins de k personnes ne diffuse ni ses lignes santé et justice, ni son logement, ni « dont parcours de soin » (cases « s », zéros compris) ; sous 20 accueillis, la Partie 1 passe par `appliquerKAnonymat` de la synthèse (réutilisée, tables de règles propres au formulaire) ; mention de **diffusion restreinte** sur chaque page du PDF et en ligne `#` du CSV dès qu'une case publiée compte moins de 5 ; encadré de l'écran avant « Générer » (`GET /convergence/parametres`). **Frein judiciaire non transmis par défaut** (correctif B-02, `insertion.cvg_transmettre_justice` à `false` : colonne non lue, ligne « non transmis (art. 10 RGPD) »). RQTH tirée du texte de paie par **liste blanche** (`utils/rqth.js`, M-02) ; une source illisible donne des cellules vides, jamais 0 (M-01).
- **Réglages** (`insertion-settings.js`) : `insertion.cvg_frein_seuil` (3), `insertion.cvg_sortie_delai_jours` (30), `insertion.cvg_sans_bilan_est_sans_nouvelles` (true), `insertion.cvg_k_min` (5), `insertion.cvg_transmettre_justice` (false), `insertion.cvg_sortie_depuis` (absent : aucune borne de mise en service).
- **RGPD** : les instantanés Convergence sont purgés par la **même purge** que la synthèse de dialogue de gestion (11ᵉ, `rgpd.dialogues_gestion_retention_jours`, six ans — une seule table, une seule règle) ; le **registre des moyens humains** a sa propre purge (**12ᵉ**, `purgeCvgRessources`, `rgpd.cvg_ressources_retention_jours`, trois ans après la fin d'une ressource — correctif M-03) ; la situation de sortie Convergence par salarié, son **historique** (`insertion_sortie_cvg_history`, auteur initial conservé et `modifie_par` distinct — m-03) et la ligne du registre rattachée au salarié sont purgés à l'anonymisation (`services/anonymization.js`). Codes de journal : `INSERTION_CVG_APERCU/_GENERATION/_CONSULTATION/_COMPARAISON`, `EXPORT_CVG`, `INSERTION_SORTIE_CVG_ECRITURE` (bloquants) ; `INSERTION_SORTIE_CVG_LECTURE`, `INSERTION_CVG_COMPLETUDE`, `INSERTION_CVG_RESSOURCE_CREATION/_MODIFICATION/_SUPPRESSION` (tolérants, écrans internes) ; `AUTO_PURGE_CVG_RESSOURCES`, `PURGE_CVG_RESSOURCES` (libellés dans `frontend/src/utils/rgpd-libelles.js`).
- **Ce qui reste hors logiciel** : SOLIDATA produit les valeurs et un PDF au même format que le classeur Convergence, il ne remplit pas un fichier Excel du réseau — aucune trame numérique n'a été transmise, seulement un document scanné.
...
| Art. 15 | Droit d'accès — **le candidat lui-même**, sans compte | `GET /api/pcm/sessions/:token/restitution` | Oui (2.45.0, voir § 2.2.3) |
| Art. 12-14 | Information préalable (test de personnalité) | `POST /api/pcm/sessions/:token/notice` | Oui (2.45.0, bloquante) |
...
| — | Purges de rétention (registre `PURGES_RGPD`) | `GET /api/rgpd/purges`, `POST /api/rgpd/purges/:cle/executer` | Oui (8 purges, dont 2 propres au PCM) |
