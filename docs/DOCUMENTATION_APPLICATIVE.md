...
- **Doctrine** : remplissage **permanent**, jamais une campagne — chaque information se saisit là où elle naît (diagnostic à l'entrée, bilan de sortie à la sortie), l'onglet Convergence ne fait que composer, comparer et transmettre. Un instantané enregistré est un **snapshot rejouable à l'identique**, distinct de ce que le dossier dit aujourd'hui. **Aucun seuil de k-anonymat** sur ce document (à la différence de la synthèse de dialogue de gestion) : le format du réseau porte lui-même des effectifs de 1 et 2 — arbitrage direction/DPO ouvert.
- **Réglages** (`insertion-settings.js`) : `insertion.cvg_frein_seuil` (3), `insertion.cvg_sortie_delai_jours` (30), `insertion.cvg_sans_bilan_est_sans_nouvelles` (true).
- **RGPD** : les instantanés Convergence sont purgés par la **même purge** que la synthèse de dialogue de gestion (11ᵉ, `rgpd.dialogues_gestion_retention_jours`, six ans — une seule table, une seule règle) ; la situation de sortie Convergence par salarié est purgée à l'anonymisation (`services/anonymization.js`). Codes de journal ajoutés : `INSERTION_CVG_APERCU/_GENERATION/_CONSULTATION/_COMPARAISON`, `EXPORT_CVG`, `INSERTION_SORTIE_CVG_ECRITURE` (libellés dans `frontend/src/utils/rgpd-libelles.js`).
