...
*Document préparé pour une réunion de présentation d'une heure. Version du 14 septembre 2026.*

> **Comment se déroule concrètement cette réunion.** Un déroulé pas à pas, condition par condition,
> avec les chemins de clic exacts, est disponible dans
> [`PRESENTATION_AUTORITE_DEMONSTRATION.md`](PRESENTATION_AUTORITE_DEMONSTRATION.md). Ce document-ci
> reste la présentation à lire avant la séance ; l'autre est le script du jour même.
...
Première mise en service le **8 mars 2026** ; version en service au 14 septembre 2026 : la **2.55.0**.
...
| 3 | Bouton **« Fiche parcours (PDF) »** (en tête de fiche) → la fiche de parcours complète s'ouvre dans une fenêtre d'impression au format A4 | 20 s |
| 4 | Onglet **Diagnostic** → **« PDF dossier »** → le diagnostic d'accueil complet | 20 s |
| 5 | Onglet **Suivi** → chaque entretien → **« PDF dossier »** (un bilan = un document daté) ; le cas échéant **« Bilan de prolongation (PDF) »** du Pass IAE (bouton en tête de fiche) | 20 s par pièce |
...
Six documents, dont les spécifications exactes (colonnes, sections, en-tête de traçabilité) nous ont
été dictées par votre service (`09-matrice-reporting-autorite.md` § 2) — elles priment sur toute
interprétation de notre part.

| Document | Fréquence | Qui peut le générer | État au 14 septembre 2026 |
|---|---|---|---|
| **(a) Export FSE+ participants** (CSV, nominatif, un par projet et par trimestre) | Trimestriel, et à toute demande de bilan | ADMIN, RH | **Livré** (chantier A) |
| **(b) Bilan d'exécution agrégé** (PDF, strictement non nominatif, signé par la direction) | À chaque bilan intermédiaire et au solde | ADMIN, RH | **Livré** (chantier A) |
| **(c) Feuille de temps** (PDF + CSV, par intervenant et par mois, jamais le nom du bénéficiaire) | Mensuelle, close au 10 du mois suivant | ADMIN, RH ; chaque intervenant voit la sienne | **Livré** (chantier B) |
| **(d) Tableau des freins enrichi** (XLSX/CSV, nominatif, ne sort de la structure que sous la forme agrégée) | Annuelle, et à la demande en contrôle sur place | ADMIN, RH | **Livré** pour les 23 colonnes historiques ; les colonnes supplémentaires que vous demandez (BRSA, catégorie France Travail, référent, freins à l'entrée et leur évolution) sont **en cours de livraison** (chantier D) |
| **(e) Synthèse de dialogue de gestion** (PDF + CSV, strictement non nominatif, agrégats seuls) | Annuelle, transmise 15 jours avant la séance ; version trimestrielle allégée pour les publics à l'entrée et la conformité | ADMIN, RH, MANAGER (lecture agrégée seule) | **En cours de livraison** (chantier D) — la synthèse pour comité de pilotage existante (§ 15 du guide CIP) en tient lieu dans l'intervalle, sur un périmètre plus étroit |
...
« Document de travail ERP — les saisies officielles font foi », et un **refus explicite** — jamais un
fichier vide — si son périmètre ne ramène aucune ligne. Les exclusions absolues (frein santé, frein
judiciaire, note de profil, notes de suivi) s'appliquent à tous les six, sans exception non plus.
...
| **4** | **Générez devant moi l'export participants et la synthèse de dialogue de gestion, montrez la ligne de journal, puis provoquez un export vide.** | Les deux exports produits en séance, puis la ligne inscrite à l'instant dans le journal d'audit (date, compte, nombre de lignes). Puis un export sur un périmètre sans participant : **refus motivé, aucun fichier produit**. L'export participants est livré depuis le chantier A. *La synthèse de dialogue de gestion en neuf blocs est le livrable du chantier D, en cours de livraison au 14 septembre 2026 (§ « Ce que vous recevrez ») : si la séance la précède, nous montrerons la synthèse existante et le dirons — le déroulé exact est dans [`PRESENTATION_AUTORITE_DEMONSTRATION.md`](PRESENTATION_AUTORITE_DEMONSTRATION.md).* |
...
complété le même jour de la livraison du chantier B (cadre RSA et temps d'accompagnement — § 2, § 3,
§ 9, § 12), et **relu et complété le 14 septembre 2026** à la livraison du chantier D (reporting
autorité et présentation) : les neuf questions du § 7.2 de votre rapport ont été reprises une par une
contre le code effectivement livré des chantiers A à D (§ 1 à § 10 ci-dessus), la section
« Ce que vous recevrez » détaille les six exports demandés, et trois documents séparés répondent aux
trois autres demandes du chantier D — [`MATRICE_QUI_VOIT_QUOI_INSERTION.md`](MATRICE_QUI_VOIT_QUOI_INSERTION.md)
(condition 3), [`PRESENTATION_AUTORITE_DEMONSTRATION.md`](PRESENTATION_AUTORITE_DEMONSTRATION.md) (le
déroulé pas à pas des cinq conditions), et [`PIECES_HORS_LOGICIEL_INSERTION.md`](PIECES_HORS_LOGICIEL_INSERTION.md)
(condition 5). Aucune donnée nominative n'y figure ; les copies d'écran sont des maquettes renseignées
...
MDFSE+ — est dit comme tel, pas comme réglé, de même que ce que le chantier D laisse en cours (la
synthèse de dialogue de gestion elle-même, § « Ce que vous recevrez »). Nous nous engageons à dater
chaque affirmation de nos prochaines notes : « livré le… » ou « prévu pour… ».*
