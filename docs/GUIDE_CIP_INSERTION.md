...
- **Où travailler** : votre page de travail est **« Espace CIP (insertion) »** (menu RH et Insertion). La page **« Pilotage & indicateurs »** sert à la direction et au dialogue de gestion — vous n'avez pas besoin de l'ouvrir tous les jours.
...
4. Vérifiez l'en-tête de fiche : Pass IAE, prescripteur, durée cumulée CDDI. Complétez ce qui manque.
...
> - **Judiciaire** : n'écrivez **jamais la nature des faits ni les condamnations**. Uniquement le niveau de frein et l'impact concret (« contrainte horaire liée à une obligation judiciaire »). Un rappel s'affiche à la saisie ; ce volet n'est visible que de vous, de la RH et de l'administrateur, et n'est **jamais** proposé automatiquement.
...
2. Si le badge **« préparation prête »** est présent, la note de préparation IA a été générée automatiquement 7 jours avant le rendez-vous (option activable dans les réglages). Ouvrez-la : synthèse de situation, points à aborder selon le type d'entretien, questions suggérées.
3. Sinon, cliquez **« Préparer cet entretien »** : la génération prend jusqu'à deux minutes — anticipez, ne la lancez pas à 13 h 58.
4. La note est **une proposition** : modifiez-la, supprimez ce qui ne va pas, ajoutez vos points. Votre version est conservée avec l'entretien (`ia_preparation`, historisée).

**Ce qui est automatique** : la génération à J-7 pour les entretiens planifiés (désactivable dans les réglages), la synthèse factuelle sans IA, la **pseudonymisation** — l'IA ne reçoit **jamais** le nom du salarié ni ses données d'identification, seulement un dossier anonymisé (« Salarié A »).
...
1. Depuis la fiche : **« Nouveau bilan »** (ou ouvrez le bilan planifié). Le bilan est un **« Bilan n° N »** créable à n'importe quelle date (les anciens M+3 / M+6 / M+10 sont conservés avec leur titre d'origine). Le formulaire suit **l'ordre de la trame papier**, en cinq étapes affichées sur un rail latéral :
   ...
   5. **Clôture** — progression et autonomie, points de vigilance (champ interne, jamais sur l'exemplaire salarié), avis global.
...
> - **Un bilan clôturé est verrouillé.** Pour corriger une erreur : « Rouvrir » (ADMIN/RH), avec un **motif obligatoire** — la réouverture est journalisée et les validations sont à refaire. C'est ce qui donne sa valeur de preuve au dossier en cas de contrôle.
...
5. Plus tard, au calme, complétez depuis le tableau **Actions CIP** : **partenaire mobilisé** (CAF, France Travail, CPAM, ANTS, SOLIHA, bailleur, OPCO, mission locale, SPIP… — liste administrable), **résultat**, **durée passée**, rattachement à un objectif ou à un bilan si utile.
...
**Ce qui est automatique** : le contrôle des **bornes légales** — au plus 1 mois par convention, cumul limité à **60 jours sur 12 mois glissants chez un même organisme d'accueil**, au plus 2 conventions avec le même organisme (et pour des objets différents). En cas de dépassement, le module vous avertit en citant la règle (refus documenté). Le contrat CDDI **n'est pas suspendu** pendant l'immersion — rien à faire côté paie.
...
3. Depuis la fiche (badge Pass) **ou** depuis la liste « Pass à préparer » : **« Bilan de prolongation »**. Le module assemble un PDF (`GET /pass-iae/bilan/:employeeId`) à partir des bilans déjà saisis : synthèse du parcours, évolution des freins, actions menées et envisagées — le support attendu par le prescripteur habilité.
...
3. **Pilotage & indicateurs** (direction, dialogue de gestion) : **indicateurs conventionnels** en tête (taux de sorties par catégorie comparés aux **cibles conventionnelles** paramétrées — affichées « objectif non paramétré » tant que la direction n'a pas confirmé les valeurs, c'est normal), ETP réalisés (étiquetés « contrôle — la saisie officielle reste l'ASP »), typologie des publics, délai moyen des diagnostics, réalisation des jalons par échéance, cartographie 9 freins, files actives, renouvellements et Pass à préparer, satisfaction et post-sortie. Rapport IA de situation globale (ADMIN/RH), export PDF A4.
4. **Export « Tableau des freins » (23 colonnes)** : le tableau demandé par le cahier des charges (identité, dates ACI et Pass, RQTH, niveau de formation, ressources, logement, situation familiale, les freins, PMSMP, projet de formation, emploi visé), en XLSX ou CSV. Avant de générer, le module affiche la **complétude** (`/insertion-freins/completude`) et des **filtres** (année, présents/sortis, par CIP référente). **Chaque génération est journalisée** (qui, quand, quel périmètre — exigence RGPD).
5. **Exports pour l'extérieur** : la **variante agrégée non nominative** — synthèse comité de pilotage (`/insertion-synthese`, PDF/CSV). L'export **FSE+** trimestriel (questionnaires d'entrée/sortie des participants CDDI) est disponible, le cofinancement FSE+ étant confirmé.

**Ce qui est automatique** : le calcul de tous les indicateurs à partir de vos saisies courantes (aucune saisie « pour les statistiques »), la valorisation des freins de l'export à la **dernière évaluation en date** (LATERAL sur le dernier entretien réalisé, repli sur le diagnostic), la **journalisation** de chaque export nominatif.

> **Points de vigilance**
> - Par défaut, l'export 23 colonnes **exclut le frein judiciaire** ; la variante « colonnes sensibles » (`sensibles=1`) reste réservée ADMIN/RH et est journalisée distinctement. Pour toute transmission externe, utilisez la variante **agrégée**.
...
1. Sur chaque diagnostic et chaque bilan clôturé, le bouton **PDF** propose deux versions :
   - **Exemplaire salarié** : ouvre sur « **Ce que nous avons décidé** » en trois phrases courtes, l'évolution des freins en pictogrammes (↗ s'améliore, ↘ se dégrade, = stable), la toile d'araignée, un gros corps de texte (FALC). **Sans** les points de vigilance internes ni les détails santé/judiciaire.
...
3. Faites signer l'exemplaire salarié papier (double signature bilan ; triple signature renouvellement). Enregistrez la **remise** (date + mode : en main propre / envoyé — `remise_salarie`) et, si possible, joignez le **scan de l'exemplaire signé** à l'entretien.
...
Pas directement : un bilan clôturé est verrouillé (c'est sa valeur de preuve). Utilisez « Rouvrir » (ADMIN/RH) avec un motif — la réouverture est journalisée, les validations existantes tombent et sont à refaire après correction.
