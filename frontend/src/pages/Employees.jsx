...
export default function Employees() {
  ...
  const loadData = async () => {
    try {
      ...
      const [empRes, teamRes, posRes] = await Promise.all([
        ...
        api.get('/teams'),
        api.get('/referentiels/positions'),
      ]);
  ...
  const createEmployee = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ first_name: '', last_name: '', email: '', phone: '', gender: '', position_id: '', team_id: '', contract_type: 'CDI', hire_date: '' });
      loadData();
    } catch (err) { console.error(err); }
  };
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500">Gestion des employés — {employees.length} collaborateur{employees.length > 1 ? 's' : ''}</p>
          </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="text-left p-3 text-xs font-semibold text-gray-500">Date embauche</th>
            ...
            <tbody>
              {employees.map(emp => (
                <tr key={emp.id} className="border-t hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(emp)}>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-xs text-gray-500">{emp.hire_date ? new Date(emp.hire_date).toLocaleDateString('fr-FR') : '—'}</td>
                  <td className="p-3">
                  ...
                  <td className="p-3">
                    <button className="text-solidata-green text-xs font-medium hover:underline">Détails</button>
                  </td>
                </tr>
              ))}
              {employees.length === 0 && (
          ...
          <div className="fixed inset-0 bg-black/30 flex justify-end z-50" onClick={() => setSelected(null)}>
            <div className="bg-white w-[420px] h-full overflow-y-auto shadow-xl p-6" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold">Fiche collaborateur</h2>
                <button onClick={() => setSelected(null)} className="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
              </div>
              <div className="flex items-center gap-4 mb-6">
                <div className="w-16 h-16 rounded-full bg-solidata-green/20 text-solidata-green flex items-center justify-center text-xl font-bold">
              ...
              <div className="space-y-3 text-sm">
                ...
                <Field label="Date d'embauche" value={selected.hire_date ? new Date(selected.hire_date).toLocaleDateString('fr-FR') : null} />
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <form onSubmit={createEmployee} className="bg-white rounded-xl p-6 w-[440px] shadow-xl max-h-[90vh] overflow-y-auto">
