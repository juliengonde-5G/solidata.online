    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-orange-950 to-slate-900 text-white p-6 flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-6 flex-1 min-h-0">
        <div className="col-span-2 bg-white/5 backdrop-blur rounded-2xl p-4 border border-white/10">
          <h3 className="text-lg font-semibold text-slate-200 mb-2">CA & tickets par heure <span className="text-xs font-normal text-slate-500">(heure de Paris)</span></h3>
        ...
        <div className="bg-white/5 backdrop-blur rounded-2xl p-4 border border-white/10 flex flex-col">
          <h3 className="text-lg font-semibold text-slate-200 mb-3">Dernières ventes <span className="text-xs font-normal text-slate-500">(heure de Paris)</span></h3>
          {ticker.length === 0 ? (
                ...
                <li key={t.id || i} className={`bg-white/5 rounded-lg px-3 py-2 flex items-center justify-between ${i === 0 ? 'ring-2 ring-orange-400 animate-pulse-once' : ''}`}>
                  <div>
                    <div className="text-xs text-slate-400">
                      {/* date_ticket est stocké en UTC → affichage en heure de Paris (Lot 12), cohérent avec les graphes par heure (agrégés Europe/Paris côté SQL). */}
                      {new Date(t.date_ticket || Date.now()).toLocaleTimeString('fr-FR', { timeZone: 'Europe/Paris' })}
                    </div>
