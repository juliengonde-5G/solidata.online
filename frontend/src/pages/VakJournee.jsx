    ...
    <div className="bg-white rounded-card shadow-card overflow-hidden border border-slate-100">
      ...
      {!hasData ? (
        ...
        <div className="p-4">
          ...
          {/* Bandeau premières / dernières ventes — timestamps stockés en UTC,
              affichés en heure de Paris (Lot 12), cohérent avec les graphes par
              heure (agrégés Europe/Paris côté SQL). */}
            ...
            <div className="bg-slate-50 rounded-lg px-3 py-2 mb-4 text-xs text-slate-600 flex justify-between">
              <span>
                ...
                <strong className="text-slate-800">
                  {k.premiere_vente ? new Date(k.premiere_vente).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Paris' }) : '—'}
                </strong>
              </span>
              <span>
                Dernière vente :{' '}
                <strong className="text-slate-800">
                  {k.derniere_vente ? new Date(k.derniere_vente).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Paris' }) : '—'}
                </strong>
              </span>
            </div>
          )}
          ...
          <div className="mb-4">
            <h4 className="text-xs uppercase tracking-wider text-slate-500 mb-1">Activité par heure <span className="normal-case text-slate-400">(heure de Paris)</span></h4>
