...
describe('CONTRAT GET /insertion/audit — blocs conventionnel/typologies/contrôle (PR 2 — EXG-10/24)', () => {
  ...
  it('typologies NON nominatives : RQTH (avec repli texte paie), ressources, niveaux, TRANCHES d’âge (jamais la date de naissance)', async () => {
    ...
    // CORRECTIF B-02 — un MANAGER n'obtenait ni le compte RQTH ni les ressources
    // perçues. Rôle RETIRÉ sur main (10/09/2026, fusion du 25/09/2026) : la
    // projection `projeterAuditPourRole` reste en place (garde morte), ce qui
    // est observable est le refus 403 — aucune typologie servie.
    const rm = await get('/api/insertion/audit?year=2026', 'RETIRE');
    expect(rm.status).toBe(403);
    expect(rm.body.typologies).toBeUndefined();
